...
function outingTrainingWithSelection(difficulty_choice, province_choice, selectedNames, inspireTalents){
  ...
  // use province-specific cost per talent if configured
  const pType_local = (target && target.type) ? target.type : '普通省';
  const cfg_local = (typeof PROVINCE_INSPIRE_SETTINGS !== 'undefined' && PROVINCE_INSPIRE_SETTINGS[pType_local]) ? PROVINCE_INSPIRE_SETTINGS[pType_local] : { maxSelect:2, activationProb:0.3, costPerTalent:12000 };
  const talentInspireCost = inspireTalents.length * (cfg_local.costPerTalent || 12000);
  ...
  for(let s of selectedStudents){
    ...
    // 新模型：极小固定收益 + 按学生出发前实力的比例收益
    const outfitEffectMult = (typeof OUTFIT_EFFECT_MULTIPLIER !== 'undefined' ? OUTFIT_EFFECT_MULTIPLIER : 1.0);

    // 计算学生当前的平均知识与能力值
    const ks_ds = Number(s.knowledge_ds || 0);
    const ks_graph = Number(s.knowledge_graph || 0);
    const ks_str = Number(s.knowledge_string || 0);
    const ks_math = Number(s.knowledge_math || 0);
    const ks_dp = Number(s.knowledge_dp || 0);
    const avgKnowledge = (ks_ds + ks_graph + ks_str + ks_math + ks_dp) / 5.0;
    const avgAbility = ((Number(s.thinking) || 0) + (Number(s.coding) || 0)) / 2.0;
    const combinedStat = avgKnowledge + avgAbility;

    // 固定部分按难度档次选择
    const fixedKnowledge = (difficulty_choice===2) ? OUTFIT_FIXED_KNOWLEDGE_INTERMEDIATE : (difficulty_choice===3) ? OUTFIT_FIXED_KNOWLEDGE_ADVANCED : OUTFIT_FIXED_KNOWLEDGE_BASIC;
    const fixedAbility = (difficulty_choice===2) ? OUTFIT_FIXED_ABILITY_INTERMEDIATE : (difficulty_choice===3) ? OUTFIT_FIXED_ABILITY_ADVANCED : OUTFIT_FIXED_ABILITY_BASIC;

    // 比例部分基于学生当前综合实力
    const propKnowledge = Math.floor(combinedStat * OUTFIT_PROP_KNOWLEDGE_RATIO);
    const propAbility = combinedStat * OUTFIT_PROP_ABILITY_RATIO;

    // 最终增益（受档次/省份/不匹配修正与全局 OUTFIT_EFFECT_MULTIPLIER 影响）
    const knowledge_gain = Math.floor((fixedKnowledge + propKnowledge) * knowledge_modifier * outfitEffectMult);
    ...
    const ability_gain = (fixedAbility + propAbility) * ability_modifier * outfitEffectMult;
    ...
    if(inspireTalents && inspireTalents.length > 0){
      for(const talentName of inspireTalents){
        const prob = (cfg_local && typeof cfg_local.activationProb === 'number') ? cfg_local.activationProb : 0.3;
        if(Math.random() < prob){
  ...
  if(talentInspireCost > 0){
    log(`天赋激发费用：¥${talentInspireCost}（${inspireTalents.length}个天赋 × ¥${cfg_local.costPerTalent || 12000}）`);
  }
...
function weeklyUpdate(weeks=1){
  ...
  // 声誉吸引：高声誉时可能有天赋较高的学生主动前来
  try{ if(typeof tryReputationAttract === 'function') tryReputationAttract(); }catch(e){ console.error('rep attract trigger error', e); }
...
window.onload = ()=>{
  ...
  if(document.getElementById('action-train')){
    ...
    const actionTrainBtn = document.getElementById('action-train');
    if(actionTrainBtn) actionTrainBtn.onclick = ()=>{ trainStudentsUI(); };
    const actionEntertainBtn = document.getElementById('action-entertain');
    if(actionEntertainBtn) actionEntertainBtn.onclick = ()=>{ entertainmentUI(); };
    const actionMockBtn = document.getElementById('action-mock');
    if(actionMockBtn) actionMockBtn.onclick = ()=>{ holdMockContestUI(); };
    const actionOutingBtn = document.getElementById('action-outing');
    if(actionOutingBtn) actionOutingBtn.onclick = ()=>{ outingTrainingUI(); };
    const buyBtn = document.getElementById('action-buy-student');
    if(buyBtn) buyBtn.onclick = ()=>{ buyStudentUI(); };
    document.getElementById('action-resign').onclick = ()=>{ resignUI(); };

    // ---------- 学生市场 / 购买功能 ----------
    function generateMarketCandidates(count, opts){
      opts = opts || {};
      const arr = [];
      for(let i=0;i<(count||3);i++){
        const name = typeof generateName === 'function' ? generateName() : ('学生' + Math.floor(getRandom()*1000));
        // base ability biased by reputation: higher reputation -> better pool
        const rep = Number(game.reputation || 0);
        const base = 40 + (rep/100)*40; // 40..80
        const spread = 20;
        const thinking = clamp(Math.round(normal(base, spread)), 10, 120);
        const coding = clamp(Math.round(normal(base, spread)), 10, 120);
        const mental = clamp(Math.round(normal(base, spread)), 10, 120);
        const studentDef = { name, thinking, coding, mental, talents: new Set() };
        // higher rep increases chance of talents, but filter out internal/debug talents
        const talentChance = Math.min(0.6, 0.05 + rep/200);
        if(window.TalentManager){
          const registered = window.TalentManager.getRegistered() || [];
          for(const tName of registered){
            if(!tName) continue;
            // skip internal or cleanup tokens
            if(tName.startsWith('__')) continue;
            const tDef = window.TalentManager.getTalent(tName) || {};
            if(tDef.beneficial === false) continue; // skip negative
            const p = Number(tDef.prob || 0) || 0;
            // combine base talent prob with market talentChance, clamp
            if(getRandom() < Math.min(1, p + talentChance * 0.18)){
              studentDef.talents.add(tName);
            }
          }
        }
        // compute a simple talent quality score (ability average + talent count bonus)
        const abilityAvg = ((studentDef.thinking||0) + (studentDef.coding||0) + (studentDef.mental||0))/3.0;
        const talentCount = (studentDef.talents && studentDef.talents.size) ? studentDef.talents.size : 0;
        const talentScore = abilityAvg + talentCount * 15; // talent adds to score
        studentDef._talentScore = Math.round(talentScore);
        // assign tier probabilistically
        // support a 'refresh' bias where refreshes should favor lower-tier candidates
        let rnd = getRandom();
        let pLow, pMid, pHigh;
        if(opts && opts.refresh){
          // On refresh, greatly favor low-tier to avoid repeatedly showing many mid/high candidates
          pLow = 0.70; pMid = 0.25; pHigh = 0.05;
          // do NOT apply strong talentScore boosts when refreshing to keep refresh cheaper
        } else {
          // base probabilities: low 40%, mid 40%, high 20%
          pLow = 0.40; pMid = 0.40; pHigh = 0.20;
          // bias by talentScore (non-refresh generation can still bias towards stronger candidates)
          if(studentDef._talentScore >= 85){ pHigh += 0.25; pMid -= 0.10; pLow -= 0.15; }
          else if(studentDef._talentScore >= 70){ pHigh += 0.10; pMid += 0.10; pLow -= 0.20; }
          else if(studentDef._talentScore < 50){ pLow += 0.25; pMid -= 0.10; pHigh -= 0.15; }
        }
        // normalize
        const sumP = Math.max(1e-6, pLow + pMid + pHigh);
        pLow /= sumP; pMid /= sumP; pHigh /= sumP;
        if(rnd < pLow) studentDef._tier = '低';
        else if(rnd < pLow + pMid) studentDef._tier = '中';
        else studentDef._tier = '高';
        arr.push(studentDef);
      }
      return arr;
    }

    function computeStudentPrice(def){
      try{
        const abilityAvg = ((def.thinking||0) + (def.coding||0) + (def.mental||0))/3.0;
        const talentCount = (def.talents && def.talents.size) ? def.talents.size : 0;
        const tier = def._tier || '中';
        // 大幅上调价格：将基价与倍率大幅提高
        let base = 0;
        if(tier === '低') base = 12000;
        else if(tier === '中') base = 30000;
        else if(tier === '高') base = 90000;
        // talent count and ability additive; add a premium multiplier for high tier
        let price = Math.floor(base + abilityAvg * 800 + talentCount * 15000);
        if(tier === '高') price = Math.floor(price * 1.4);
        // final clamp
        price = Math.max(2000, Math.min(99999999, price));
        return price;
      }catch(e){ return 10000; }
    }

    function buyStudentUI(){
      let candidates = generateMarketCandidates(4);
      const cards = candidates.map((c,idx)=>{
        const price = computeStudentPrice(c);
        const talents = Array.from(c.talents || []).slice(0,3).map(t=>`<span class="talent-tag" style="margin-right:6px">${t}</span>`).join('');
        const tierLabel = c._tier ? `<span style="display:inline-block;padding:2px 6px;border-radius:6px;background:${c._tier==='高'? '#ffd54f': c._tier==='中'? '#90caf9' : '#e0e0e0'};color:#333;margin-right:8px;font-size:12px">${c._tier}级</span>` : '';
        return `
          <div class="market-card option-card" data-idx="${idx}" style="padding:10px;border-radius:8px;border:1px solid #eee;min-width:220px;">
            <div style="font-weight:700;margin-bottom:6px">${tierLabel}${c.name} — ¥${price}</div>
            <div class="small muted">思维 ${c.thinking} · 编程 ${c.coding} · 心理 ${c.mental}</div>
            <div style="margin-top:8px">${talents}</div>
          </div>`;
      }).join('');

      const refreshLabel = (typeof MARKET_REFRESH_COST !== 'undefined') ? `刷新候选 (¥${MARKET_REFRESH_COST})` : '刷新候选';
      const html = `<h3>购买学生市场</h3>
        <div class="small muted">从其他学校或自由市场购买学生。价格由能力与天赋决定。</div>
        <div id="market-grid" style="display:flex;gap:12px;overflow-x:auto;margin-top:12px;max-height:320px;">${cards}</div>
        <div class="modal-actions" style="margin-top:12px">
          <button class="btn btn-ghost" onclick="closeModal()">取消</button>
          <button class="btn" id="market-refresh">${refreshLabel}</button>
        </div>`;
      showModal(html);
      ...
      function bindCards(){
        const cardEls = Array.from(document.querySelectorAll('#market-grid .market-card'));
        if(cardEls.length && !cardEls[0].classList.contains('selected')) cardEls[0].classList.add('selected');
        cardEls.forEach(el => {
          el.onclick = ()=>{
            cardEls.forEach(x=>x.classList.remove('selected'));
            el.classList.add('selected');
          };
          const idx = parseInt(el.dataset.idx,10);
          el.ondblclick = ()=>{ attemptBuy(idx); };
        });
      }

      function attemptBuy(idx){
        const def = candidates[idx];
        if(!def) return;
        const price = computeStudentPrice(def);
        if(game.budget < price){ alert('经费不足，无法购买该学生'); return; }
        if(!confirm(`购买 ${def.name} 将花费 ¥${price}，确认购买？`)) return;
        // perform purchase
        game.recordExpense(price, `购买学生 ${def.name}`);
        const newStud = new Student(def.name, def.thinking, def.coding, def.mental);
        if(def.talents && def.talents.size){ for(const t of def.talents) newStud.addTalent(t); }
        game.students.push(newStud);
        // 授予该购买学生当前/下一个比赛的晋级资格（自动入选本赛季的下一场比赛）
        // NOTE: 为平衡，低阶学生购买时有概率不会自动获得晋级资格；中阶亦有较小概率不获资格
        try{
          const comps = Array.isArray(window.competitions) ? window.competitions.slice().sort((a,b)=>a.week-b.week) : [];
          const nextComp = comps.find(c => c.week > game.week);
          // 计算赛季索引
          ...
          let seasonIdx_local = Math.floor((game.week - 1) / Math.max(1, weeksPerSeason_local));
          if(seasonIdx_local < 0) seasonIdx_local = 0; if(seasonIdx_local >= seasonsCount_local) seasonIdx_local = seasonsCount_local - 1;
            if(nextComp){
            if(!game.qualification) game.qualification = [];
            if(!game.qualification[seasonIdx_local]){ game.qualification[seasonIdx_local] = {}; for(const nm of COMPETITION_ORDER) game.qualification[seasonIdx_local][nm] = new Set(); }
            if(!game.qualification[seasonIdx_local][nextComp.name]) game.qualification[seasonIdx_local][nextComp.name] = new Set();
            // determine chance to grant qualification based on tier
            let grantProb = 1.0;
            const tier = def._tier || '中';
            if(tier === '低') grantProb = 0.40; // 40% chance to get qualification
            else if(tier === '中') grantProb = 0.85; // 85% chance
            else if(tier === '高') grantProb = 1.0; // always
            if(Math.random() < grantProb){
              game.qualification[seasonIdx_local][nextComp.name].add(newStud.name);
              pushEvent({ name: '学生购买', description: `${newStud.name} 已加入队伍（购买），并获得 ${nextComp.name} 的晋级资格`, week: game.week });
            } else {
              pushEvent({ name: '学生购买', description: `${newStud.name} 已加入队伍（购买），但未获 ${nextComp.name} 的晋级资格`, week: game.week });
            }
          } else {
            pushEvent({ name: '学生购买', description: `${newStud.name} 已加入队伍（购买）`, week: game.week });
          }
        }catch(e){
          pushEvent({ name: '学生购买', description: `${newStud.name} 已加入队伍（购买）`, week: game.week });
          console.error('为购买学生添加晋级资格时出错', e);
        }
        closeModal();
        renderAll();
      }
      ...
      if(refresh) refresh.onclick = ()=>{
        // 在模态内刷新候选：优先生成低档候选，降低中/高级出现概率
        try{
          // charge refresh cost if configured
          if(typeof MARKET_REFRESH_COST !== 'undefined'){
            if(game.budget < MARKET_REFRESH_COST){ alert('经费不足，无法刷新候选'); return; }
            game.recordExpense(MARKET_REFRESH_COST, '刷新学生候选');
            try{ renderAll(); }catch(e){}
          }
          candidates = generateMarketCandidates(4, { refresh: true });
          const cards = candidates.map((c,idx)=>{
            const price = computeStudentPrice(c);
            const talents = Array.from(c.talents || []).slice(0,3).map(t=>`<span class="talent-tag" style="margin-right:6px">${t}</span>`).join('');
            const tierLabel = c._tier ? `<span style="display:inline-block;padding:2px 6px;border-radius:6px;background:${c._tier==='高'? '#ffd54f': c._tier==='中'? '#90caf9' : '#e0e0e0'};color:#333;margin-right:8px;font-size:12px">${c._tier}级</span>` : '';
    ...
    // 声誉吸引事件：高声誉时每周有概率产生高天赋学生主动加入
    function tryReputationAttract(){
      try{
        const rep = Number(game.reputation || 0);
        if(rep < 60) return; // 低于阈值很少有人主动来
        const chance = Math.min(0.6, (rep - 50) / 120 + 0.05); // gradually increases with rep
        if(getRandom() < chance){
          // generate a high-quality candidate
          const candidates = generateMarketCandidates(1);
          const def = candidates[0];
          // boost abilities and talents
          def.thinking = Math.round(Math.max(def.thinking, 60 + (rep-60)));
          def.coding = Math.round(Math.max(def.coding, 60 + (rep-60)));
          def.mental = Math.round(Math.max(def.mental, 50));
          // ensure has at least one good talent
          if(def.talents.size === 0 && window.TalentManager){
            const reg = window.TalentManager.getRegistered() || [];
            for(const t of reg){ if(getRandom() < 0.08){ def.talents.add(t); if(def.talents.size >= 2) break; } }
          }
          // push event with accept/decline
          const name = def.name;
          const talentList = Array.from(def.talents || []).slice(0,3).join('、') || '无';
          const desc = `${name} 听闻贵校声誉，有意加盟（思维 ${def.thinking} / 编程 ${def.coding} / 天赋: ${talentList}）。是否接纳？`;
          pushEvent({ name: '潜在加盟', description: desc, week: game.week, options: [ { label: '接纳', effect: ()=>{
              try{
                const newStud = new Student(def.name, def.thinking, def.coding, def.mental);
                if(def.talents && def.talents.size) for(const t of def.talents) newStud.addTalent(t);
                game.students.push(newStud);
                // 授予该加盟学生下一个比赛的晋级资格（如果存在）
                const comps_local = Array.isArray(window.competitions) ? window.competitions.slice().sort((a,b)=>a.week-b.week) : [];
                const nextComp_local = comps_local.find(c => c.week > game.week);
                const seasonsCount_local2 = (typeof SEASON_REPLICATION_COUNT !== 'undefined') ? SEASON_REPLICATION_COUNT : (typeof window.SEASONS_COUNT !== 'undefined' ? window.SEASONS_COUNT : 2);
                const weeksPerSeason_local2 = (typeof WEEKS_PER_SEASON !== 'undefined') ? WEEKS_PER_SEASON : (typeof WEEKS_PER_HALF !== 'undefined' ? WEEKS_PER_HALF : Math.floor((typeof SEASON_WEEKS !== 'undefined' ? SEASON_WEEKS : 32)/seasonsCount_local2));
                let seasonIdx_local2 = Math.floor((game.week - 1) / Math.max(1, weeksPerSeason_local2));
                if(seasonIdx_local2 < 0) seasonIdx_local2 = 0; if(seasonIdx_local2 >= seasonsCount_local2) seasonIdx_local2 = seasonsCount_local2 - 1;
                if(nextComp_local){ if(!game.qualification) game.qualification = []; if(!game.qualification[seasonIdx_local2]){ game.qualification[seasonIdx_local2] = {}; for(const nm of COMPETITION_ORDER) game.qualification[seasonIdx_local2][nm] = new Set(); } if(!game.qualification[seasonIdx_local2][nextComp_local.name]) game.qualification[seasonIdx_local2][nextComp_local.name] = new Set(); game.qualification[seasonIdx_local2][nextComp_local.name].add(def.name); }
                pushEvent({ name: '加盟成功', description: `${def.name} 已加入队伍`, week: game.week });
                renderAll();
              }catch(e){ console.error('接纳加盟处理出错', e); pushEvent({ name: '加盟成功', description: `${def.name} 已加入队伍`, week: game.week }); renderAll(); }
            } }, { label: '婉拒', effect: ()=>{ pushEvent({ name: '婉拒加盟', description: `你婉拒了 ${def.name} 的加盟邀请`, week: game.week }); } } ], eventId: `rep_attract_${Date.now()}` });
        }
      }catch(e){ console.error('tryReputationAttract error', e); }
    }
