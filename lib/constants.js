...
// 赛季复制次数：将原始比赛日程复制为多个赛季（用于支持多赛季模式）
// 将其设置为 6 以支持最多 6 个赛季（可调整）
const SEASON_REPLICATION_COUNT = 6;
/* 能力与知识权重 */
const KNOWLEDGE_WEIGHT = 0.6;
const ABILITY_WEIGHT = 0.4;
// 当思维/代码超过该阈值时，对后续的增幅进行衰减（例如 models.addThinking/addCoding 使用）
const ABILITY_DECAY_THRESHOLD = 4000;
/* 压力/恢复 */
...
/* 省份基础 */
const STRONG_PROVINCE_BUDGET = 20000000;
const NORMAL_PROVINCE_BUDGET = 10000000;
const WEAK_PROVINCE_BUDGET = 4000000;
...
// 提高中级与高级的基价以提升资源消耗权衡
const OUTFIT_BASE_COST_INTERMEDIATE = 40000;
const OUTFIT_BASE_COST_ADVANCED = 120000;
// 强省对集训的费用溢价略微上调，弱省折扣略微下调
const STRONG_PROVINCE_COST_MULTIPLIER = 1.8;
const WEAK_PROVINCE_COST_MULTIPLIER = 0.6;
// 知识基准保持不变或小幅调整（可按需微调）
const OUTFIT_KNOWLEDGE_BASE_BASIC = 8;
const OUTFIT_KNOWLEDGE_BASE_INTERMEDIATE = 12;
const OUTFIT_KNOWLEDGE_BASE_ADVANCED = 20;
// 降低中级/高级的能力基准以减少其相对强度
const OUTFIT_ABILITY_BASE_BASIC = 12.0;
const OUTFIT_ABILITY_BASE_INTERMEDIATE = 16.0;
const OUTFIT_ABILITY_BASE_ADVANCED = 26.0;
const OUTFIT_PRESSURE_BASIC = 30;
const OUTFIT_PRESSURE_INTERMEDIATE = 50;
const OUTFIT_PRESSURE_ADVANCED = 75;
// 新的集训收益模型：极小固定收益 + 基于学生当前实力的比例收益
// 固定部分（按档次）
const OUTFIT_FIXED_KNOWLEDGE_BASIC = 2;
const OUTFIT_FIXED_KNOWLEDGE_INTERMEDIATE = 3;
const OUTFIT_FIXED_KNOWLEDGE_ADVANCED = 4;
const OUTFIT_FIXED_ABILITY_BASIC = 1.0;
const OUTFIT_FIXED_ABILITY_INTERMEDIATE = 1.5;
const OUTFIT_FIXED_ABILITY_ADVANCED = 2.0;
// 比例部分：按学生当前综合实力（思维+编码+平均知识）的一定比例给出增益
// 例如 0.04 表示按综合实力的 4% 作为知识增益（再乘以档次/其他修正）
const OUTFIT_PROP_KNOWLEDGE_RATIO = 0.04;
const OUTFIT_PROP_ABILITY_RATIO = 0.05;
/* 声誉对外出集训/商业/媒体的影响系数 */
// 声誉越高，外出集训费用越低：最大折扣比例（相对于原价）
const OUTFIT_REPUTATION_DISCOUNT = 0.90; // 最高可减免 90%
...
/* 市场刷新与省份激活配置 */
// 刷新学生候选的基础费用（每次刷新消耗经费）
const MARKET_REFRESH_COST = 8000;

// 不同省份对天赋激活的限制与费用：
// key 为省份 type（'强省' / '普通省' / '弱省'），值为 { maxSelect, activationProb, costPerTalent }
const PROVINCE_INSPIRE_SETTINGS = {
  // 激活概率有所下调以降低天赋激活率
  '强省': { maxSelect: 4, activationProb: 0.50, costPerTalent: 18000 },
  '普通省': { maxSelect: 2, activationProb: 0.30, costPerTalent: 12000 },
  '弱省': { maxSelect: 1, activationProb: 0.15, costPerTalent: 8000 }
};
...
// 稍微降低训练默认收益以缓和成长曲线（可在运行时覆盖）
TRAINING_EFFECT_MULTIPLIER = 0.90;      // 训练效果增幅（训练后属性增加），默认下降 10%
