...
(function(global){
  ...
  // 全局辅助：根据周数计算赛季索引（支持多赛季配置）
  function __getSeasonIndex(week){
    const seasonsCount = (typeof SEASON_REPLICATION_COUNT !== 'undefined') ? SEASON_REPLICATION_COUNT : (typeof window.SEASONS_COUNT !== 'undefined' ? window.SEASONS_COUNT : 2);
    const weeksPerSeason = (typeof WEEKS_PER_SEASON !== 'undefined') ? WEEKS_PER_SEASON : ((typeof WEEKS_PER_HALF !== 'undefined') ? WEEKS_PER_HALF : Math.floor((typeof SEASON_WEEKS !== 'undefined' ? SEASON_WEEKS : 32) / seasonsCount));
    let idx = Math.floor((Number(week||1) - 1) / Math.max(1, weeksPerSeason));
    if(idx < 0) idx = 0;
    if(idx >= seasonsCount) idx = seasonsCount - 1;
    return idx;
  }
  ...
  const halfIndex = (typeof __getSeasonIndex === 'function') ? __getSeasonIndex(game.week) : 0;
    
    // 初始化晋级资格数据结构（保证当前赛季对象存在）
    if(!game.qualification) game.qualification = [];
    const seasonsCount_check = (typeof SEASON_REPLICATION_COUNT !== 'undefined') ? SEASON_REPLICATION_COUNT : (typeof window.SEASONS_COUNT !== 'undefined' ? window.SEASONS_COUNT : 2);
    if(!game.qualification[halfIndex]){
      game.qualification[halfIndex] = {};
      for(let nm of COMPETITION_ORDER) game.qualification[halfIndex][nm] = new Set();
    }
  ...
  // 使用统一的赛季索引计算函数
  const seasonIdx = (typeof __getSeasonIndex === 'function') ? __getSeasonIndex(game.week) : (function(){ const sc = (typeof SEASON_REPLICATION_COUNT !== 'undefined') ? SEASON_REPLICATION_COUNT : 2; const wp = (typeof WEEKS_PER_HALF !== 'undefined') ? WEEKS_PER_HALF : Math.floor((typeof SEASON_WEEKS !== 'undefined' ? SEASON_WEEKS : 32)/sc); let i = Math.floor((game.week-1)/Math.max(1,wp)); if(i<0) i=0; if(i>=sc) i=sc-1; return i; })();
  ...
  function distributeRewards(results, compName){
    ...
    try{
    const halfIndex = (typeof __getSeasonIndex === 'function') ? __getSeasonIndex(game.week) : (function(){ const sc = (typeof SEASON_REPLICATION_COUNT !== 'undefined') ? SEASON_REPLICATION_COUNT : 2; const wp = (typeof WEEKS_PER_HALF !== 'undefined') ? WEEKS_PER_HALF : Math.floor((typeof SEASON_WEEKS !== 'undefined' ? SEASON_WEEKS : 32)/sc); let i = Math.floor((game.week-1)/Math.max(1,wp)); if(i<0) i=0; if(i>=sc) i=sc-1; return i; })();
