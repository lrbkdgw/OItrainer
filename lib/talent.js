  ...
  checkAndHandleTalentLoss(student) {
    ...
    registerDefaultTalents(game, utils){
      ...
      this.registerTalent({
        ...
        // ======= 新增天赋（第三批） =======
        // 分块思维：当题目分档较多时，思维在本题临时提升，帮助分而治之
        this.registerTalent({
          name: '分块思维',
          description: '面对多子任务题目时，思维在本题临时提升，有助于分块解题。',
          color: '#29B6F6',
          prob: 0.06,
          kind: 'positive',
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(!ctx || !ctx.state) return null;
              const pid = ctx.problemId;
              const probObj = ctx.state.getProblem && ctx.state.getProblem(pid);
              if(!probObj || !Array.isArray(probObj.subtasks)) return null;
              if(probObj.subtasks.length < 3) return null; // 仅针对分档较多的题
              if(!student._talent_backup['分块思维']){
                student._talent_backup['分块思维'] = { thinking: Number(student.thinking || 0) };
                student.thinking = Number(student.thinking || 0) * 1.4;
                student._talent_state['分块思维'] = true;
                return '分块思维发动：本题思维 +40%（本题临时）';
              }
            }catch(e){ console.error('分块思维 天赋错误', e); }
            return null;
          }
        });

        // 速效学习：训练立即获得较高短期收益，但会额外增加压力
        this.registerTalent({
          name: '速效学习',
          description: '训练时可能短期内效率极高，但会额外增加压力（适合短期冲刺）。',
          color: '#FF7043',
          prob: 0.05,
          kind: 'positive',
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(!(src === 'training' || src === 'task_training' || (typeof src === 'string' && src.indexOf('train') === 0))) return null;
              if(getRandom() < 0.25){
                // 返回标准 action 供外部训练系统识别：增加短期知识比率并额外增加压力
                return { action: 'quick_study', knowledgeMultiplier: 1.5, extraPressure: 8, message: '速效学习：短期知识增幅+50%，压力+8' };
              }
            }catch(e){ console.error('速效学习 天赋错误', e); }
            return null;
          }
        });

        // 技术宅：比赛开始时编码能力永久性（本场）提升
        this.registerTalent({
          name: '技术宅',
          description: '比赛开始时编码能力显著提升，适合偏重编码的选手。',
          color: '#3949AB',
          prob: 0.09,
          kind: 'positive',
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ensureTemp(student);
              if(eventName !== 'contest_start') return null;
              if(!student._talent_backup['技术宅']){
                student._talent_backup['技术宅'] = { coding: Number(student.coding || 0) };
                student.coding = Number(student.coding || 0) * 1.25;
                student._talent_state['技术宅'] = true;
                return '技术宅发动：编码 +25%（本场临时）';
              }
            }catch(e){ console.error('技术宅 天赋错误', e); }
            return null;
          }
        });

        // 社交达人：在团队事件/娱乐后更易获得好感度与舒适度恢复
        this.registerTalent({
          name: '社交达人',
          description: '擅长社交，参与娱乐/团队活动后更快恢复压力并有小概率触发积极事件。',
          color: '#8E24AA',
          prob: 0.07,
          kind: 'positive',
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              if(eventName !== 'entertainment_finished' && eventName !== 'team_event') return null;
              // 在娱乐结束时额外减压并有概率产生正面事件
              student.pressure = Math.max(0, Number(student.pressure || 0) - 5);
              if(getRandom() < 0.20){
                return { action: 'social_bonus', message: '社交达人：活动后额外减压并触发正面事件' };
              }
              return '社交达人：活动后压力-5';
            }catch(e){ console.error('社交达人 天赋错误', e); }
            return null;
          }
        });

        // 拖延症：在应当训练或完成任务时有概率拖延，导致本次无训练增益且压力上升
        this.registerTalent({
          name: '拖延症',
          description: '计划外的拖延会使训练无效并导致额外压力。',
          color: '#BDBDBD',
          prob: 0.06,
          kind: 'negative',
          beneficial: false,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(!(src === 'training' || src === 'task_training' || (typeof src === 'string' && src.indexOf('train') === 0))) return null;
              if(getRandom() < 0.20){
                // 返回 reject_training 表示外部训练系统应取消本次增益
                student.pressure = Math.min(100, Number(student.pressure || 0) + 10);
                return { action: 'reject_training', message: '拖延症：训练无效，压力+10' };
              }
            }catch(e){ console.error('拖延症 天赋错误', e); }
            return null;
          }
        });

        // 返工狂：通过子任务后有小概率额外返工修正，获得额外少量知识但消耗时间（外部需处理消耗）
        this.registerTalent({
          name: '返工狂',
          description: '通过子任务后会检查改进，有小概率获得额外知识，但需要消耗额外时间。',
          color: '#4DB6AC',
          prob: 0.05,
          kind: 'positive',
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              if(eventName !== 'contest_pass_subtask') return null;
              if(getRandom() < 0.15){
                // 返回 action 以便外部计时系统扣除时间并给予知识收益
                return { action: 'rework_gain', knowledgeBonus: 0.5, timeCostTicks: 1, message: '返工狂：额外改进，知识+0.5，但消耗时间' };
              }
            }catch(e){ console.error('返工狂 天赋错误', e); }
            return null;
          }
        });
        ...
        // ========== 新增天赋集合（第四批：25个） ==========
        // 1. 逆向思维：面对难题时更擅长逆向分析，思维临时提升
        this.registerTalent({
          name: '逆向思维',
          description: '擅长从结论反推，面对复杂子任务时思维临时提升。',
          color: '#1976D2',
          prob: 0.06,
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              const pid = ctx && ctx.problemId;
              const state = ctx && ctx.state;
              const probObj = state && state.getProblem && state.getProblem(pid);
              if(!probObj) return null;
              // 如果题目最后一档分值高于平均，视为适合逆向
              const avg = (probObj.subtasks || []).reduce((s,x)=>s+Number((x&&x.score)||0),0)/Math.max(1,(probObj.subtasks||[]).length);
              const last = (probObj.subtasks || []).slice(-1)[0];
              if(last && last.score > avg){
                if(!student._talent_backup['逆向思维']){
                  student._talent_backup['逆向思维'] = { thinking: Number(student.thinking||0) };
                  student.thinking = Number(student.thinking||0) * 1.3;
                  student._talent_state['逆向思维'] = true;
                  return '逆向思维发动：思维 +30%（本题临时）';
                }
              }
            }catch(e){ console.error('逆向思维 天赋错误', e); }
            return null;
          }
        });
        ...
        this.registerTalent({
          name: '数学怪才',
          description: '数学题目中临时提升数学知识与思维。',
          color: '#FFB300',
          prob: 0.05,
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{ ensureTemp(student); if(eventName !== 'contest_select_problem') return null; const state = ctx && ctx.state; const pid = ctx && ctx.problemId; const p = state && state.getProblem && state.getProblem(pid); if(!p || !p.tags || !p.tags.includes('数学')) return null; if(!student._talent_backup['数学怪才']){ student._talent_backup['数学怪才'] = { knowledge_math: Number(student.knowledge_math||0), thinking: Number(student.thinking||0) }; student.knowledge_math = Number(student.knowledge_math||0) * 1.5; student.thinking = Number(student.thinking||0) * 1.2; student._talent_state['数学怪才'] = true; return '数学怪才发动：数学知识 +50%，思维 +20%（本题临时）'; } }catch(e){ console.error('数学怪才 天赋错误', e); } return null; }
        });

        // 3. 代码匠人：编码检定成功率提升
        this.registerTalent({
          name: '代码匠人',
          description: '编码检定成功率提高，在 coding 检定时减难。',
          color: '#303F9F',
          prob: 0.07,
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{ if(eventName !== 'contest_check_subtask') return null; if(!ctx || ctx.checkType !== 'coding') return null; return { action: 'reduce_difficulty', amount: 0.12, message: '代码匠人：代码难度-12%' }; }catch(e){ console.error('代码匠人 天赋错误', e); } return null; }
        });

        // 4. 运气女神：随机运气型天赋，在比赛中小概率触发正向事件
        this.registerTalent({
          name: '运气女神',
          description: '极小概率触发幸运事件，如随机通过一档或获得小量分数。',
          color: '#F06292',
          prob: 0.03,
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{ if(eventName !== 'contest_thinking') return null; if(getRandom() < 0.02){ return { action: 'lucky_event', message: '运气女神眷顾：触发小幅正向效果' }; } }catch(e){ console.error('运气女神 天赋错误', e); } return null; }
        });

        // 5. 懒散：训练效果降低
        this.registerTalent({
          name: '懒散',
          description: '学习效率较低，训练增益降低。',
          color: '#9E9E9E',
          prob: 0.06,
          kind: 'negative',
          beneficial: false,
          handler: function(student, eventName, ctx){ try{ if(eventName !== 'pressure_change') return null; if(!ctx) return null; const src = ctx.source||''; if(!(src==='training' || src==='task_training' || (typeof src==='string' && src.indexOf('train')===0))) return null; return { action: 'reduce_all_gain', amount: 0.25, message: '懒散：训练增益-25%' }; }catch(e){ console.error('懒散 天赋错误', e);} return null; }
        });

        // 6. 夜猫子：夜间训练有效，但白天效率降低（需要外部时间判定）
        this.registerTalent({
          name: '夜猫子',
          description: '在深夜训练效率高，但白天会疲惫（外部时间系统处理）。',
          color: '#6A1B9A',
          prob: 0.05,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ if(eventName !== 'pressure_change') return null; if(!ctx) return null; const time = ctx.timeOfDay || 'day'; if(time === 'night'){ return { action:'increase_gain', amount:0.30, message:'夜猫子：夜间训练增益+30%'}; } else { return { action:'reduce_gain', amount:0.10, message:'夜猫子：白天训练增益-10%'}; } }catch(e){ console.error('夜猫子 天赋错误', e);} return null; }
        });

        // 7. 早起鸟：早晨训练效率高
        this.registerTalent({
          name: '早起鸟',
          description: '清晨训练获得额外增益（外部时间系统配合）。',
          color: '#FFEE58',
          prob: 0.06,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ if(eventName!=='pressure_change') return null; if(!ctx) return null; const time = ctx.timeOfDay||'day'; if(time==='morning'){ return { action:'increase_gain', amount:0.20, message:'早起鸟：清晨训练增益+20%'}; } }catch(e){ console.error('早起鸟 天赋错误', e);} return null; }
        });

        // 8. 细节狂：通过子任务时更有可能取得高分
        this.registerTalent({
          name: '细节狂',
          description: '注重细节，通过子任务时有小概率获得额外分值。',
          color: '#455A64',
          prob: 0.05,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ if(eventName!=='contest_pass_subtask') return null; if(getRandom()<0.15) return { action:'extra_point', amount:2, message:'细节狂：额外分数+2' }; }catch(e){ console.error('细节狂 天赋错误', e);} return null; }
        });

        // 9. 宏观规划：训练计划效果延续更久（被动标记）
        this.registerTalent({
          name: '宏观规划',
          description: '训练效果持续时间延长，长期收益更明显。',
          color: '#00796B',
          prob: 0.06,
          beneficial: true,
          handler: function(student, eventName, ctx){ return null; }
        });

        // 10. 临场发挥：比赛中低概率大幅提升单次能力
        this.registerTalent({
          name: '临场发挥',
          description: '比赛中有小概率短时大幅提升能力。',
          color: '#D81B60',
          prob: 0.04,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ ensureTemp(student); if(eventName!=='contest_thinking') return null; if(getRandom()<0.08){ if(!student._talent_backup['临场发挥']){ student._talent_backup['临场发挥'] = { thinking: Number(student.thinking||0), coding: Number(student.coding||0) }; student.thinking = Number(student.thinking||0) * 1.8; student.coding = Number(student.coding||0) * 1.5; student._talent_state['临场发挥']=true; return '临场发挥发动：思维与编码大幅提升（短时）'; } } }catch(e){ console.error('临场发挥 天赋错误', e);} return null; }
        });

        // 11. 稳重如山：减缓压力上升
        this.registerTalent({
          name: '稳重如山',
          description: '压力增长时减缓压力上升速率。',
          color: '#546E7A',
          prob: 0.07,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ if(eventName!=='pressure_change') return null; if(!ctx || typeof ctx.amount==='undefined') return null; const amt = Number(ctx.amount)||0; if(amt>0) return { action:'reduce_pressure', amount:0.25, message:'稳重如山：本次压力增幅-25%' }; }catch(e){ console.error('稳重如山 天赋错误', e);} return null; }
        });

        // 12. 创意狂人：解题时有小概率发现更优策略
        this.registerTalent({
          name: '创意狂人',
          description: '在思考阶段有小概率发现更优策略，减少难度或加速通过。',
          color: '#FF7043',
          prob: 0.05,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ if(eventName!=='contest_thinking') return null; if(getRandom()<0.10) return { action:'reduce_difficulty', amount:0.2, message:'创意狂人：策略优化，难度-20%' }; }catch(e){ console.error('创意狂人 天赋错误', e);} return null; }
        });

        // 13. 数据图谱：提交与调试时更擅长数据结构相关思路
        this.registerTalent({
          name: '数据图谱',
          description: '数据结构题中有较高概率触发知识加成。',
          color: '#0288D1',
          prob: 0.06,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ ensureTemp(student); if(eventName!=='contest_select_problem') return null; const state = ctx && ctx.state; const pid = ctx && ctx.problemId; const p = state && state.getProblem && state.getProblem(pid); if(!p||!p.tags||!p.tags.includes('数据结构')) return null; if(!student._talent_backup['数据图谱']){ student._talent_backup['数据图谱'] = { knowledge_ds: Number(student.knowledge_ds||0) }; student.knowledge_ds = Number(student.knowledge_ds||0) * 1.4; student._talent_state['数据图谱']=true; return '数据图谱发动：数据结构 +40%（本题临时）'; } }catch(e){ console.error('数据图谱 天赋错误', e);} return null; }
        });

        // 14. 记忆大师：复盘后获得更持久的知识增幅（被动标记）
        this.registerTalent({
          name: '记忆大师',
          description: '复盘与总结后训练效果更加持久。',
          color: '#8D6E63',
          prob: 0.06,
          beneficial: true,
          handler: function(student, eventName, ctx){ return null; }
        ...
        // 15. 速记术：短时间记忆提升，比赛中快速学习小量知识
        this.registerTalent({
          name: '速记术',
          description: '比赛期间短时间内能快速吸收少量知识。',
          color: '#FFA000',
          prob: 0.05,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ if(eventName!=='contest_thinking') return null; if(getRandom()<0.12){ const keys=['knowledge_ds','knowledge_graph','knowledge_string','knowledge_math','knowledge_dp']; const k=keys[Math.floor(getRandom()*keys.length)]; student[k] = Number(student[k]||0) + 0.5; return '速记术：临时学习+0.5'; } }catch(e){ console.error('速记术 天赋错误', e);} return null; }
        });

        // 16. 抗疲劳：长时间比赛中保持能力（被动标记）
        this.registerTalent({
          name: '抗疲劳',
          description: '比赛疲劳衰减效果减弱，耐力更强。',
          color: '#6D4C41',
          prob: 0.07,
          beneficial: true,
          handler: function(student, eventName, ctx){ return null; }
        ...
        // 17. 节奏大师：在固定节奏训练/比赛时性能更佳（需要外部节奏信息）
        this.registerTalent({
          name: '节奏大师',
          description: '按计划训练时获得额外加成。',
          color: '#C2185B',
          prob: 0.05,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ if(eventName==='pressure_change' && ctx && ctx.onSchedule) return { action:'increase_gain', amount:0.15, message:'节奏大师：按计划训练增益+15%'}; }catch(e){ console.error('节奏大师 天赋错误', e);} return null; }
        });

        // 18. 焦虑症：比赛中压力更容易上升（负面）
        this.registerTalent({
          name: '焦虑症',
          description: '比赛或训练中更容易积累压力。',
          color: '#B71C1C',
          prob: 0.04,
          kind: 'negative',
          beneficial: false,
          handler: function(student, eventName, ctx){ try{ if(eventName==='pressure_change' && ctx && typeof ctx.amount!=='undefined'){ return { action:'increase_pressure', amount:0.20, message:'焦虑症：压力增加幅度+20%'}; } }catch(e){ console.error('焦虑症 天赋错误', e);} return null; }
        });

        // 19. 极限冲刺：比赛末段短期能力爆发
        this.registerTalent({
          name: '极限冲刺',
          description: '比赛末段短时大幅提升能力，但结束后会疲惫。',
          color: '#E64A19',
          prob: 0.05,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ ensureTemp(student); if(eventName==='contest_select_problem'){ const state=ctx && ctx.state; if(!state) return null; const remaining = state.problems.filter(p=>!p.solved).length; if(remaining<=1 && !student._talent_backup['极限冲刺']){ student._talent_backup['极限冲刺'] = { thinking: Number(student.thinking||0), coding: Number(student.coding||0) }; student.thinking = Number(student.thinking||0)*2.0; student.coding = Number(student.coding||0)*1.5; student._talent_state['极限冲刺']=true; return '极限冲刺发动：短时能力爆发'; } } }catch(e){ console.error('极限冲刺 天赋错误', e);} return null; }
        });

        // 20. 稳步成长：比赛结束后长期获得少量经验（被动标记）
        this.registerTalent({
          name: '稳步成长',
          description: '长期稳定成长，比赛与训练的长期收益略高。',
          color: '#2E7D32',
          prob: 0.08,
          beneficial: true,
          handler: function(student, eventName, ctx){ return null; }
        ...
        // 21. 团队领袖：队伍中其他成员在团队活动中获得小幅加成
        this.registerTalent({
          name: '团队领袖',
          description: '在团队活动中为队友带来加成（外部系统需应用）。',
          color: '#2962FF',
          prob: 0.05,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ if(eventName==='team_event'){ return { action:'team_boost', amount:0.1, message:'团队领袖：队友获得+10%效果' }; } }catch(e){ console.error('团队领袖 天赋错误', e);} return null; }
        });

        // 22. 独行侠：个人比赛表现提升，但团队活动效果减弱
        this.registerTalent({
          name: '独行侠',
          description: '个人比赛中表现更好，但团队活动收益降低。',
          color: '#455A64',
          prob: 0.05,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ if(eventName==='contest_start'){ return { action:'personal_boost', amount:0.15, message:'独行侠：个人表现+15%'}; } if(eventName==='team_event'){ return { action:'reduce_team_benefit', amount:0.3, message:'独行侠：团队活动收益-30%'}; } }catch(e){ console.error('独行侠 天赋错误', e);} return null; }
        });

        // 23. 求稳党：总是选择最低风险策略，减少失败代价
        this.registerTalent({
          name: '求稳党',
          description: '选择保守策略，降低失败时的惩罚。',
          color: '#78909C',
          prob: 0.07,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ if(eventName==='contest_finish' && ctx && ctx.state){ if(ctx.state.totalPenalty) return { action:'reduce_penalty', amount:0.4, message:'求稳党：失败惩罚-40%'} } }catch(e){ console.error('求稳党 天赋错误', e);} return null; }
        });

        // 24. 冒险家：偏好高风险高回报策略
        this.registerTalent({
          name: '冒险家',
          description: '更倾向于尝试高难度档位，成功时奖励更高。',
          color: '#F4511E',
          prob: 0.05,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ if(eventName==='contest_check_subtask' && ctx && typeof ctx.difficulty!=='undefined'){ return { action:'risk_prefer', multiplier:1.25, message:'冒险家：偏好高风险，高回报'} } }catch(e){ console.error('冒险家 天赋错误', e);} return null; }
        });

        // 25. 精准打击：针对性地提高某一知识点临时加成
        this.registerTalent({
          name: '精准打击',
          description: '选题后针对题目知识点获得临时加成。',
          color: '#1E88E5',
          prob: 0.06,
          beneficial: true,
          handler: function(student, eventName, ctx){ try{ ensureTemp(student); if(eventName!=='contest_select_problem') return null; const state=ctx&&ctx.state; const pid=ctx&&ctx.problemId; const p=state&&state.getProblem&&state.getProblem(pid); if(!p) return null; const tag = (p.tags && p.tags[0]) || null; if(!tag) return null; const map={'数据结构':'knowledge_ds','图论':'knowledge_graph','字符串':'knowledge_string','数学':'knowledge_math','DP':'knowledge_dp'}; const key=map[tag]; if(!key) return null; const bakKey='精准打击_'+key; if(!student._talent_backup[bakKey]){ student._talent_backup[bakKey] = Number(student[key]||0); student[key] = Number(student[key]||0) * 1.5; student._talent_state['精准打击']=true; return `精准打击发动：${tag}知识 +50%（本题临时）`; } }catch(e){ console.error('精准打击 天赋错误', e);} return null; }
        });
