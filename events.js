...
(function(global){
  ...
  description: '知名教练莅临指导，学生能力提升，压力降低',
      ...
      this.register({
        ...
        run: c => {
          // 计算当前队内学生的最大能力值
      // 新增事件：短视频走红（可能带来声誉或负面关注）
      this.register({
        id: 'viral_video',
        name: '短视频走红',
        description: '学生发布的视频意外走红，可能带来声誉提升或争议',
        check: c => getRandom() < 0.015,
        run: c => {
          const good = getRandom() < 0.7;
          if(good){
            const repGain = c.utils.uniformInt(2,6);
            const money = c.utils.uniformInt(2000,8000);
            c.game.reputation = Math.min(100, (c.game.reputation||0) + repGain);
            c.game.budget = (c.game.budget||0) + money;
            const msg = `学生短视频走红，声誉 +${repGain}，获得赞助 ¥${money}`;
            c.log && c.log(`[短视频] ${msg}`);
            window.pushEvent && window.pushEvent({ name: '短视频走红', description: msg, week: c.game.week });
          } else {
            const repLoss = c.utils.uniformInt(3,8);
            c.game.reputation = Math.max(0, (c.game.reputation||0) - repLoss);
            const msg = `学生短视频引发争议，声誉 -${repLoss}`;
            c.log && c.log(`[短视频负面] ${msg}`);
            window.pushEvent && window.pushEvent({ name: '短视频争议', description: msg, week: c.game.week });
          }
          return null;
        }
      });

      // 新增事件：校友捐赠（小幅资金与一次性声誉加成）
      this.register({
        id: 'alumni_donation',
        name: '校友捐赠',
        description: '成功的校友回馈母校，赠予经费或设备',
        check: c => getRandom() < 0.01 && (c.game.reputation || 0) >= 30,
        run: c => {
          const money = c.utils.uniformInt(10000, 40000);
          c.game.budget = (c.game.budget||0) + money;
          c.game.reputation = Math.min(100, (c.game.reputation||0) + 2);
          const msg = `校友捐赠 ¥${money}，声誉 +2`;
          c.log && c.log(`[校友捐赠] ${msg}`);
          window.pushEvent && window.pushEvent({ name: '校友捐赠', description: msg, week: c.game.week });
          return null;
        }
      });

      // 新增事件：学生作弊被查（负面，可能失去晋级资格）
      this.register({
        id: 'cheating_scandal',
        name: '学生作弊',
        description: '比赛/训练中出现作弊事件，可能导致声誉下降或取消资格',
        check: c => getRandom() < 0.008 && c.game.students && c.game.students.length > 0,
        run: c => {
          // 随机挑选 1 名学生触发
          const pool = c.game.students.filter(s=>s && s.active !== false);
          if(pool.length === 0) return null;
          const idx = c.utils.uniformInt(0, pool.length-1);
          const s = pool[idx];
          const punish = getRandom() < 0.6;
          if(punish){
            c.game.reputation = Math.max(0, (c.game.reputation||0) - c.utils.uniformInt(5,12));
            // 取消该学生在当前赛季的所有晋级资格
            try{
              const seasonIdx = (typeof window.WEEKS_PER_SEASON !== 'undefined') ? Math.floor((c.game.week-1)/window.WEEKS_PER_SEASON) : 0;
              if(c.game.qualification && c.game.qualification[seasonIdx]){
                for(const nm of Object.keys(c.game.qualification[seasonIdx]||{})){
                  try{ c.game.qualification[seasonIdx][nm].delete(s.name); }catch(e){}
                }
              }
            }catch(e){}
            const msg = `${s.name} 涉及作弊事件，被取消本赛季相关晋级资格，声誉下降`;
            c.log && c.log(`[作弊] ${msg}`);
            window.pushEvent && window.pushEvent({ name: '学生作弊', description: msg, week: c.game.week });
          } else {
            const msg = `${s.name} 被误传作弊，最终澄清并未受处罚`;
            window.pushEvent && window.pushEvent({ name: '疑似作弊', description: msg, week: c.game.week });
          }
          return null;
        }
      });

      // 新增事件：主教练受伤（短期无法训练）、研究合作机会（选择事件）
      this.register({
        id: 'coach_injury',
        name: '教练受伤',
        description: '主教练意外受伤，短期内训练效果下降或受限制',
        check: c => getRandom() < 0.006,
        run: c => {
          const weeks = c.utils.uniformInt(1,3);
          c.game.coach_injury_weeks = weeks;
          const msg = `主教练受伤，未来 ${weeks} 周训练效率下降`;
          c.log && c.log(`[教练受伤] ${msg}`);
          window.pushEvent && window.pushEvent({ name: '教练受伤', description: msg, week: c.game.week });
          return null;
        }
      });
      ...
      this.register({
        ...
        name: '研究合作',
        description: '与高校/企业达成研究合作，可选择接受或放弃',
        check: c => getRandom() < 0.01 && (c.game.reputation||0) >= 40,
        run: c => {
          const money = c.utils.uniformInt(5000,20000);
          const options = [
            { label: '接受合作', effect: ()=>{ c.game.budget = (c.game.budget||0) + money; c.game.reputation = Math.min(100, (c.game.reputation||0) + 2); window.pushEvent && window.pushEvent({ name: '合作接受', description: `接受合作：经费 +¥${money}，声誉 +2`, week: c.game.week }); }},
            { label: '婉拒', effect: ()=>{ c.game.reputation = Math.max(0, (c.game.reputation||0) - 1); window.pushEvent && window.pushEvent({ name: '合作拒绝', description: `婉拒合作：声誉 -1`, week: c.game.week }); }}
          ];
          window.showChoiceModal && window.showChoiceModal({ name: '研究合作', description: `合作方提供经费 ¥${money}，是否接受？`, week: c.game.week, options });
          return null;
        }
      });
            ...
            { label: '接收', effect: () => {
                ...
                try{
                  ...
                  // 自动晋级下场比赛：将学生加入当前赛季所有已有晋级资格的比赛
                    ...
                    let halfIndex = Math.floor((c.game.week - 1) / Math.max(1, weeksPerSeason_local));
                    if(halfIndex < 0) halfIndex = 0; if(halfIndex >= seasonsCount_local) halfIndex = seasonsCount_local - 1;
